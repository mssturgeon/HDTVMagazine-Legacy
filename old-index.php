<?
	...
	$email_address = ($user->data['email_address'] == '') ? 'email address' : $user->data['email_address'];
	...
	<table class="bare" cellpadding="0" cellspacing="0"><tr>
		<td id="left">
			...
			<form name="qq" method="POST" onsubmit="return false;">
				...
				<div class="item"><span class="corners-top"><span></span></span>
					...
					<div id="ds_body" name="ds_body" style="padding:0px 10px 0 0">
						<div id="ds_instructions" style="display:none;">
							Send your question and one of our experts will answer it personally.
						</div>
						<textarea name="question" id="question" class="blur" style="display:none" tabindex="1"
							onfocus="taFocus(this)"
							onblur="taBlur(this)"
							onkeydown="adjustAreaSize(this, 40, 200)">
						</textarea>
						<div id="ds_submittedBy" style="display:none;">
							<div class="btn btn_green" style="float:right;" tabindex="3"><a href="#" onclick="submitQuestion()">Send</a><span></span></div>
							Email Address *:
							<input type="text" name="email_address" id="email_address" value="" style="width:20em;" class="blur" tabindex="2"
			...
			<div class="item" ><span class="corners-top"><span></span></span>
				...
				<a href="http://www.facebook.com/HDTVMagazine" target="_blank" style="float:left; margin-right:10px"><img src="<?=BASE_IMG_HOST?>/images/i_facebook_64.png" alt="Facebook" align="left" height="64" width="64" /></a>
			<span class="corners-bottom"><span></span></span></div>
...
#				echo getSelfAd();
	...
	<script type="text/javascript">
		...
		function submitQuestion() {
			if (question.value == '' || question.value == origText) {
				alert('Please enter your question.');
				question.focus();
				return false;
			}

			if (email_address.value !='' && !isValidEmail(email_address)) return false;

			var ip_address = document.forms['qq'].ip_address.value;
			if (email_address.value == 'email address') {email_address.value = '';}

			url = '<?=BASE_URL?>/qqsubmit.php?question='+escape(question.value)+'&email_address='+escape(email_address.value)+'&ip_address='+escape(ip_address);
...
//			location.href = url;
		...
		function init() {
			...
			instructions = document.getElementById('ds_instructions');

			email_address = document.forms['qq'].email_address;
