<?
	...
	$sql = "SELECT entry_id, entry_title, entry_blog_id FROM mt_entry WHERE entry_id = $id";
	if ($debug) {echo "$sql\n";}
	$result = mQuery($sql);
	...
	$orig_message = addslashes(file_get_contents("http://www.hdtvmagazine.com/admin/emails/content-new-multipart.php".
	"?id=". $row['entry_id'] .
	"&boundary=". $boundary));
