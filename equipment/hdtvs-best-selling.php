...
<html>
<head>
	<title>HDTV Magazine - Best Selling HDTVs</title>
...
<body><div id="body_container">
	...
	<div style="display:table;">
		...
		<div class="tab-content">
			The Best Selling HDTVs based on Amazon.com sales rank. These include HDTVs available within the past <?=$update_days?> days. The number of reviews are listed in parenthesis.<br />
			<br />
			<?
				$types = array(
				...
				foreach ($types as $key => $clause) {
					$sql = "
					SELECT a.ASIN, SalesRank, Manufacturer, Model, Title, TotalNew, TotalUsed, ListPrice, LowestNewPrice, LowestUsedPrice, AverageRating, TotalReviews, SmallImageURL as image_url
					...
					WHERE m.ASIN = a.ASIN
						...
						AND SalesRank > 0
					ORDER BY SalesRank, AverageRating DESC, TotalReviews DESC LIMIT 5";
					$result = mQuery($sql);
					...
					echo '<h2><a href="'. strtolower($key) .'.php">Best Selling '. $key .' HDTVs</a></h2>';
					...
					while ($row = mysql_fetch_assoc($result)) {
						...
						'<span style="color:black; font-weight:bold;">#'. $row['SalesRank'] .'</span>'.
						' ('. $row['TotalReviews'] .') '.
#						$rating .' ('. $row['TotalReviews'] .') '.
						'<a href="'. URL_EQUIPMENT_MODEL .'?a='. $row['ASIN'] .'&man='. $row['Manufacturer'] .'&model='. $row['Model'] .'">'. $row['Title'] .'</a>'.
						$row['SFValue'] .' - <span class="primary_bold">$'. number_format($price/100, 2) .'</span>'.
						'</div>';
					}
